...
## 2. 권한
조회(목록·상세)는 백엔드에서 **공개 GET**(`GET /faqs`, `GET /faqs/{id}`, 가드 없음)이나, 이 관리자 화면 자체는 관리자 영역(`/admin/*`)에 있어 사이드바 노출·진입이 관리자 인증에 종속된다. 쓰기(등록/수정/삭제)는 **`faq` 권한** 관리자 전용이며 마스터는 권한과 무관하게 전체 허용.
...
- 백엔드 근거: `create_faq`/`update_faq`/`delete_faq`는 `Depends(require_permission("faq"))`. `list_faqs`/`get_faq`는 가드 없음.
...
- **규칙**:
  ...
  - **등록(create) 모드**: 우측 `등록` 버튼 1개.
    ...
    - 성공 시 `POST /faqs`(payload: title·faq_type·content·`created_by=me?.id`), alert "등록이 완료되었습니다." **후 목록으로 이동**.
  - **상세(edit) 모드**: 우측에 `삭제` + `저장` 버튼.
    - `저장`: `PATCH /faqs/{id}`(title·faq_type·content), alert "저장이 완료되었습니다." **후 목록으로 이동하지 않고 현재 화면 유지**.
    - `삭제`: confirm("삭제하시겠습니까? / 삭제된 FAQ는 복구할 수 없습니다.") → 확인 시 `DELETE /faqs/{id}` → alert "삭제가 완료되었습니다." → 목록으로 이동.
...
| 등록 | `POST /faqs` (`useCreateFaq`) | `require_permission("faq")` · 작성자 이름 = 토큰 `admin.name` 스냅샷 |
| 수정 | `PATCH /faqs/{id}` (`useUpdateFaq`) | `require_permission("faq")` · 부분 업데이트(`exclude_unset`) |
| 삭제 | `DELETE /faqs/{id}` (`useDeleteFaq`) | `require_permission("faq")` · 204 |
| 현재 관리자 | `useAdminMe()` | 등록 시 `created_by`에 `me.id` 전달 |

- 뮤테이션 성공 시 목록/상세 쿼리 invalidate로 자동 갱신.

...
| 2026-07-22 | [qa-fixes](reviews/2026-07-22-qa-fixes.md) | QA 지적사항 픽스 누적 로그 — #1 LNB 로그인 버튼 gap 6→8px, #2 프로필 버튼 hover primary→platinum 통일, #3 사이드바 버튼 공통화(SidebarNavRow/SIDEBAR_ROW_BASE), #4 fixed 지도 패널 접기 시 relayout 누락, #5 fixed 검색 지오코딩 실패 시 결과없음 표시, #6 로그아웃 후 이전 사용자 세션·제안서 잔존(useLogout 훅+clearSessionId), #7 로그인 시 사이드바 도움말 중복 제거 |
